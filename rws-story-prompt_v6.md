...
**WebFetch Verification Status (MANDATORY):**
- Status: [VERIFIED / INACCESSIBLE]
- If VERIFIED: Include exact 30-word excerpt from WebFetch result showing quote in context
- If INACCESSIBLE: Note error type (403, 404, paywall, timeout) and mark for removal

**Proof-of-Presence (MANDATORY):**
- Must be exact 30-word excerpt copied from the source page via WebFetch
- Must show the quote in its surrounding context
- Format: "...surrounding text [QUOTE] surrounding text..."
- If you cannot provide this excerpt because WebFetch failed, quote MUST BE REMOVED
...
1. You MUST attempt WebFetch on EVERY source URL before adding to ledger
2. Document the WebFetch result status for every quote
3. If WebFetch fails with 403, 404, paywall, or timeout:
   - Mark status as "INACCESSIBLE: [error]"
   - Search for alternative accessible source with same quote
   - Attempt WebFetch on alternative source
   - If no accessible source exists, DELETE quote from ledger
4. Quotes based solely on search result summaries are ABSOLUTELY FORBIDDEN
5. If Proof-of-Presence field is empty or says "search results show...", quote is INVALID
6. Before moving to Quote Audit, verify: Does every quote have a 30-word WebFetch excerpt? If NO, remove it.
...
WebFetch Status (VERIFIED / INACCESSIBLE)

"Quote found on page?" (Yes/No)

If any entry shows "INACCESSIBLE" or "No":
...
- How many quotes did I attempt to verify with WebFetch?
- How many WebFetch attempts succeeded?
- How many WebFetch attempts failed?
- How many quotes am I including based on search summaries only?

If the last number is > 0, STOP and remove those quotes.
...
Proof-of-presence is missing or doesn't contain actual WebFetch excerpt

WebFetch verification status is missing

Quote audit is skipped

Any quote based solely on search result summary

7. OUTPUT ORDER (MANDATORY)

Output in this exact order:

Quote Ledger table (with WebFetch status and 30-word excerpts)

Quote Audit table (with WebFetch status column)
...
WebFetch-enforced verification preventing unverifiable quotes
